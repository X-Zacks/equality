...
cc_data = result["dimensions"].get("cost_center", {})
...
**注意**：Cost Center 数据量较大（总共有 """ + str(cc_data.get("total_cost_centers", "N/A")) + """ 个不同的成本中心），此处仅列出 Top 10 变化。

#### Top 10 增长

| Cost Center | {q1_name} | {q2_name} | 差异 | 变化率 |
|-------------|-----------|-----------|------|--------|
"""

for item in cc_data.get("top_increase", [])[:10]:
    pct_val = f"{item['pct']:.1f}%" if item.get('pct') is not None else "新增"
    md += f"| {item['cost_center']} | {fmt(item['Q3'])} | {fmt(item['Q4'])} | {fmt(item['diff'])} | {pct_val} |\n"
...
for item in cc_data.get("top_decrease", [])[:10]:
    pct_val = f"{item['pct']:.1f}%" if item.get('pct') is not None else "-100%"
    md += f"| {item['cost_center']} | {fmt(item['Q3'])} | {fmt(item['Q4'])} | {fmt(item['diff'])} | {pct_val} |\n"

md += """
**⚠️ 建议**：对变化超过 10% 的 Cost Center 逐一复核，确认是否有组织归属变更或分摊规则调整。
