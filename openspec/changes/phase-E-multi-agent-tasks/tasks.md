...
- [x] 1.1.1 定义 `TaskRuntime`（`manual` / `cron` / `subagent`）
- [x] 1.1.2 定义 `TaskState`（`queued` / `running` / `succeeded` / `failed` / `timed_out` / `cancelled` / `lost`）
- [x] 1.1.3 定义 `TaskRecord`、`TaskNotificationPolicy`、`TaskSummary`
- [x] 1.1.4 明确合法状态迁移表

### 1.2 注册中心（`packages/core/src/tasks/registry.ts`）

- [x] 1.2.1 实现 `registerTask()`
- [x] 1.2.2 实现 `transitionTask()`（非法迁移抛错）
- [x] 1.2.3 实现 `getTask()` / `listTasks()`
- [x] 1.2.4 实现 `cancelTask()` / `steerTask()`
- [x] 1.2.5 实现任务事件广播（state_changed / finished / cancelled）

### 1.3 持久化（`packages/core/src/tasks/store.ts`）

- [x] 1.3.1 JSON 快照读写
- [x] 1.3.2 Gateway 启动时恢复任务快照
- [x] 1.3.3 上次异常退出仍为 `running` 的任务标记为 `lost`
- [x] 1.3.4 debounce 写盘，避免频繁 IO
...
- [x] 1.5.1 T27 — 注册任务后初始状态为 `queued`
- [x] 1.5.2 T28 — 合法迁移：`queued → running → succeeded`
- [x] 1.5.3 T29 — 非法迁移被拒绝
- [x] 1.5.4 T30 — 启动恢复时 `running` → `lost`
- [x] 1.5.5 T31 — `cancelTask()` 将任务置为 `cancelled`
- [x] 1.5.6 T32 — `steerTask()` 将消息投递到目标任务
...
- [x] 2.1.1 定义 `FailoverReason` 与 `FailoverDecision`
- [x] 2.1.2 实现 `classifyProviderError(err)`
- [x] 2.1.3 实现策略矩阵：rate_limit / overloaded / auth / billing / network / timeout / fatal
- [x] 2.1.4 增加 provider 冷却、探测与跳过逻辑

### 2.2 thinking 渐进降级（`packages/core/src/providers/fallback.ts` 或新模块）

- [x] 2.2.1 对支持 thinking 的模型增加等级降级逻辑
- [x] 2.2.2 保证"已有稳定流输出后不再中途切模型"
- [x] 2.2.3 统一错误消息：所有候选均失败时返回明确原因摘要

### 2.3 Runner 集成（`packages/core/src/agent/runner.ts`）

- [x] 2.3.1 主 Agent 使用增强版 failover provider
- [x] 2.3.2 子 Agent / cron 任务复用同一 failover policy
- [x] 2.3.3 onDelta 增加"模型切换"提示（仅在真正切换时）

### 2.4 单元测试（`packages/core/src/__tests__/failover-policy.test.ts`）

- [x] 2.4.1 T33 — `AbortError` 不触发 failover
- [x] 2.4.2 T34 — `429` 触发 provider 冷却并切换候选
- [x] 2.4.3 T35 — `auth` 错误禁用当前 provider 并切换
- [x] 2.4.4 T36 — `billing` 错误进入长冷却
- [x] 2.4.5 T37 — 支持 thinking 的模型按等级降级
- [x] 2.4.6 T38 — 所有候选失败返回统一错误
...
- [x] 3.1.1 实现 `spawnSubagent()`
- [x] 3.1.2 实现 `listSubagents()`
- [x] 3.1.3 实现 `steerSubagent()`
- [x] 3.1.4 实现 `killSubagent()`
- [x] 3.1.5 记录父子任务 / 会话关系
- [x] 3.1.6 限制 `depth=1`，禁止孙子 Agent

### 3.2 LLM 工具桥接（`packages/core/src/tools/builtins/subagent-*.ts`）

- [x] 3.2.1 新增 `subagent_spawn` 工具定义
- [x] 3.2.2 `subagent_spawn`：创建子任务并返回 taskId
- [x] 3.2.3 新增 `subagent_list` 工具：列出当前会话相关子任务
- [x] 3.2.4 新增 `subagent_steer` 工具：向指定子任务注入方向消息
- [x] 3.2.5 新增 `subagent_kill` 工具：取消指定子任务
...
- [x] 3.5.1 T39 — `spawn` 创建子任务并返回 taskId
- [x] 3.5.2 T40 — 子任务使用独立 sessionKey
- [x] 3.5.3 T41 — `list` 返回当前父会话相关子任务
- [x] 3.5.4 T42 — `steer` 将消息投递到运行中子任务
- [x] 3.5.5 T43 — `kill` 取消任务并迁移到 `cancelled`
- [x] 3.5.6 T44 — depth>1 被拒绝
